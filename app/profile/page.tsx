...
export default function ProfilePage() {
  ...
  const handleLogout = () => {
    ...
    fetch("/api/admin/logout", { method: "POST" }); // Clear admin session too
        ...
        <div className="grid md:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-6 rounded-xl border-2 border-gray-100 shadow-sm h-fit space-y-6">
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 text-red-500 font-bold border-2 border-red-100 py-2 rounded-lg hover:bg-red-50 transition-colors"
            >
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            {loading ? (
              <div className="flex justify-center p-8">
                <Loader2 className="animate-spin w-8 h-8 text-[#006a55]" />
              </div>
            ) : orders.length === 0 ? (
              <div className="bg-white p-8 rounded-xl border-2 border-gray-100 shadow-sm text-center">
                ...
                <button 
                  onClick={() => router.push("/")}
                  className="bg-[#006a55] text-white px-6 py-2.5 rounded-xl font-bold hover:bg-[#005544] transition-colors shadow-lg shadow-[#006a55]/20"
                >
                  Start Shopping
                </button>
              </div>
              ...
              <div className="space-y-4">
                {orders.map((order) => (
                  <div key={order._id} className="bg-white p-5 rounded-xl border-2 border-gray-100 shadow-sm hover:border-[#006a55]/30 transition-all">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-1">Order ID</p>
                        <p className="font-mono text-sm font-bold text-gray-800">#{order._id.substring(order._id.length - 6).toUpperCase()}</p>
                      </div>
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Link href={`/order-success/${order._id}`}>
                          <button className="flex items-center gap-1 bg-[#006a55] text-white px-4 py-2 rounded-lg text-sm font-bold hover:bg-[#005544] transition-colors shadow-sm">
